...
internal static class AccessRulesGenerator
{
    ...
    private static async Task GenerateAreaReachRules()
    {
        ...
        var entries = from character in Characters
                      ...
                      let rule = MakeLogicRule(graphs, character, areaFrom, areaTo, logicLevel)
    ...
    private static string MakeLogicRule(Dictionary<string, BidirectionalGraph<string, AreaToArea>> graphs, string character, string from, string to, int logicLevel)
    {
        ...
        foreach (var path in graph.RankedShortestPathHoffmanPavley(_ => 1, @from, to, 4).OrderBy(_ => _.Count()))
        {
            ...
            {
                // Build an item set for each chain only if said chain has items
                itemSets.AddRange(from chain in CartesianProduct(steps)
                                  let items = chain.SelectMany(_ => _)
                                  where items.Any()
                                  select new HashSet<string>(items));
            }
        }
